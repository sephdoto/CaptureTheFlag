...
public class EditorScene extends Scene {
  ...
  private VBox createMapChooser() {
    ...
    Spinner<Integer> rowsSpinner =
        createMapSpinner(mapRoot, 1, 100, engine.tmpTemplate.getGridSize()[0]);
    createChangeListener(rowsSpinner, "Rows", false);
    controlgrid.add(rowsSpinner, 1, 0);
    Spinner<Integer> colSpinner =
        createMapSpinner(mapRoot, 1, 100, engine.tmpTemplate.getGridSize()[1]);
  ...
  private MenuButton createMenuButton() {
    ...
    mapMenuItem.setOnAction(
        event -> {
          leftPane.getChildren().clear();
          leftPane.getChildren().add(options[0]);
          mb.setText("Edit Map");
        });
    figureMenuItem.setOnAction(
        event -> {
          leftPane.getChildren().clear();
          leftPane.getChildren().add(options[1]);
          mb.setText("Add Pieces");
        });
    configMenuItem.setOnAction(
        event -> {
          leftPane.getChildren().clear();
    ...
    leftheader
        ...
        .bind(
            Bindings.createObjectBinding(
                () -> Font.font("Century Gothic", vBox.getWidth() / divider),
                vBox.widthProperty()));
    return leftheader;
  }
        ...
        .addListener(
            (observable, oldValue, newValue) -> {
              if (newValue.length() > 15) {
                textField.setText(
                    oldValue); // Wenn die Länge überschritten wird, wird der Text auf den
