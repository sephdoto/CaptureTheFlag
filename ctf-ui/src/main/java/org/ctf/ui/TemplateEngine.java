...
/**
 * Covers all tasks concerning the management of map template data required in the map editor.
 * 
 * @author aniemesc
 */
public class TemplateEngine {
  ...
  /**
   * Generates a text representation the current template that is printed in the console used for
   * testing.
   * 
   * @author aniemesc
   */
  public void printTemplate() {
    ...
    System.out.println(
        "Rows:" + tmpTemplate.getGridSize()[0] + "  Collums:" + tmpTemplate.getGridSize()[1] + "\n"
  ...
  /**
   * Updates the piece amount in the pieces hash map when a new template is loaded.
   * 
   * @author aniemesc
   */
  ...
  /**
   * Updates the tmpTemplate attribute. If this results in an invalid template the change gets
   * reversed.
   * 
   * @author aniemesc
   * @param spinner - Spinner that caused the event
   ...
   * @return boolean that indicates whether the tmpTemplate was updated
   */
  public boolean setandCheckTemplate(Spinner<Integer> spinner, Consumer<Integer> setter, int old,
      int newV) {
    setter.accept(newV);
  ...
  /**
   * Updates the amout of figures in the current template in response to the change of a Spinner. If
   * the updated map template is invalid the changes get reversed. It Also checks if there is at
  ...
  /**
   * Updates the pieces attribute and removes pieces from the template if their amount is 0. Makes
   * sure that pieces get stored properly even though they were removed from the template.
   * 
   * @author aniemesc
   * @param type
   * @param newValue
  ...
  /**
   * Fills the comboBox for custom pieces with all existing pieces that are not part of the default
   * set.
   * 
   * @author aniemesc
  ...
  /**
   * Updates the Directions attribute of the tmpTemplate in response to a Spinner change.
   * 
   * @author aniemesc
