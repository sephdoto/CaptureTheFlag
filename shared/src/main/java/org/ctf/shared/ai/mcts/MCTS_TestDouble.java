...
/**
 * This class is a replica of an older MCTS version, it is just used for testing. The comments and
 * code is outdated, only MCTS is the newest version.
 *
 * @author sistumpf
 */
public class MCTS_TestDouble {
  ...
  /**
   * Selects a node to simulate on using the UCBk formula. expands a children if a node in the chain
   * has unexpanded ones.
   *
   ...
   * @return true if player A wins the simulation (either by getting more beans or player B having
   *     no moves left), false if player B wins the simulation (either by getting more beans or
   *     player A having no moves left) default case is a heuristic. if it returns value > 0, player
   *     A is winning
   */
      ...
      for (Piece p : teams[i].getPieces()) {
        ...
        if (p.getDescription().getMovement().getDirections() != null) {
          for (int dir = 0; dir < 8; dir++)
            points[i] +=
                AI_Tools_Old.getReach(p.getDescription().getMovement().getDirections(), dir)
   ...
   * @return -1 if the game is not in a terminal state 0 - Integer.MAX_VALUE winner team id
   */
  int isTerminal(TreeNode node) {
    ...
    for (int i = node.gameState.getCurrentTeam();
        teamsLeft > 1;
        i = AI_Tools_Old.toNextTeam(node.gameState).getCurrentTeam()) {
      ...
      if (canMove) {
        ...
        AI_Tools_Old.removeTeam(node.gameState, i);
        teamsLeft--;
      }
    }
      ...
      for (int i = 0; i < parent.possibleMoves.get(piece).size(); i++) {
        ...
        if (parent.gameState.getGrid()[pos[0]][pos[1]].contains("p:")
            && !parent.gameState.getGrid()[pos[0]][pos[1]].split("p:")[1].split("_")[0].equals(
                parent.gameState.getCurrentTeam())) {
          if (AI_Tools_Old.validPos(pos, piece, parent.gameState))
            return createMoveDeleteIndex(parent, piece, i);
        }
      }
    }
  ...
  void removeTeamCheck(GameState gameState) {
    for (int i = 0; i < gameState.getTeams().length; i++) {
      ...
      if (gameState.getTeams()[i].getFlags() == 0
        ...
        AI_Tools_Old.removeTeam(gameState, i--);
      }
    }
  }
  ...
  void alterGameState(GameState gameState, Move move) {
    ...
    if (occupant.contains("p:")) {
      ...
      picked.setPosition(
          AI_Tools_Old.respawnPiecePosition(
    ...
    AI_Tools_Old.toNextTeam(gameState);
  }
