...
public class GridObjectContainer {
  ...
  public GridObjectContainer clone() {
    ...
    if(this.object == GridObjects.piece) {
      clonedPiece = new Piece();
      clonedPiece.setDescription(this.piece.getDescription());
      clonedPiece.setId(this.piece.getId());
      clonedPiece.setPosition(this.piece.getPosition().clone());
    }
    return new GridObjectContainer(this.object, this.teamId, clonedPiece);
  }
  ...
  public boolean equals(GridObjectContainer compare) {
    if(this.object != compare.object)
      return false;
    switch(this.object) {
      case block: return compare.object == GridObjects.block;
      case base: return compare.teamId == this.teamId;
      case piece: return compare.getPiece().getId().equals(this.getPiece().getId());
    }
    return true;
  }
