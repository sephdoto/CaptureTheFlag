...
class TreeNodeTest {
  ...
  void testUpdating() {
    ...
    for(int i=0; i<50 && mcts.isTerminal(node.getReferenceGameState(), new ReferenceMove(null, new int[2])) == -1; i++){
...
//      System.out.println(node.gameState.getTeams()[0].getPieces().length + "<- 0, 1 ->" + node.gameState.getTeams()[1].getPieces().length);
  ...
  void testCaptureUpdate() {
    ...
    MCTS mcts = new MCTS(node, new AIConfig());
    for(int i=0; i<1000 && mcts.isTerminal(node.getReferenceGameState(), new ReferenceMove(null, new int[2])) == -1; i++) {
      mcts.oneMove(node, node, true, new ReferenceMove(null, new int[2]));
//      node.printGrid();
    }
  }
  ...
  void testUpdatePossibleMovesAndChildren() {
    ...
    MCTS mcts = new MCTS(node, new AIConfig());
    for(; mcts.isTerminal(node.getReferenceGameState(), new ReferenceMove(null, new int[2])) == -1; ) {
      mcts.oneMove(node, node, true, new ReferenceMove(null, new int[2]));
//      node.printGrids();
    }
  }
